...
class Evaluator():
    ...
    def input_check(cls, imgF, imgA=None, imgB=None):
        if imgA is None:
            ...
            assert imgF.shape == imgA.shape == imgB.shape, print(imgF.shape, imgA.shape, imgB.shape)
            assert len(imgF.shape) == 2, 'dimension error'
    ...
    def SSIM(cls, image_F, image_A, image_B):
        cls.input_check(image_F, image_A, image_B)
        return ssim(image_F, image_A, data_range=255.0) + ssim(image_F, image_B, data_range=255.0)
