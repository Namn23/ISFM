...
class FG_SS2D(nn.Module):
    def __init__(
        ...
        self.in_proj_x = nn.Linear(self.d_model, self.d_inner * 2, bias=bias, **factory_kwargs)
        ...
        self.in_proj_x2 = nn.Linear(self.d_model, self.d_inner * 2, bias=bias, **factory_kwargs)
        ...
        self.in_proj_z = nn.Linear(self.d_inner * 2, self.d_inner * 2, bias=bias, **factory_kwargs)
        self.in_proj_g1 = nn.Linear(self.d_inner * 2, self.d_inner, bias=bias, **factory_kwargs)
        self.in_proj_g2 = nn.Linear(self.d_inner * 2, self.d_inner, bias=bias, **factory_kwargs)
        self.act = nn.SiLU()
        ...
        self.proj = nn.Sequential(
            nn.Linear(self.d_model * 2, self.d_model // 4), nn.LayerNorm(self.d_model // 4),
            nn.Linear(self.d_model // 4, self.d_inner * 2), nn.LayerNorm(self.d_inner * 2),
        )
...
class FGM(nn.Module):
    def __init__(
        ...
        self.FGS = FG_SS2D(d_model=hidden_dim, d_state=d_state, expand=mlp_ratio, dropout=attn_drop_rate,
                           **kwargs)
    ...
    def forward(self, input, input2, LF_fuse, HF_fuse, blk_index, x_size):
        ...
        x = input * self.skip_scale + input2 * self.skip_scale_2 + self.drop_path(self.FGS(x, input2, LF_fuse, HF_fuse))  # text_guidance
